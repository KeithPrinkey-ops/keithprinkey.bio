        ...
        <div class="relative">
            ...
            <div class="relative px-6 py-16 ml-auto mr-auto bg-top bg-cover sm:py-20 max-w-7xl md:px-24 lg:px-16 lg:py-24">
                <div class="text-center">
                    ...
                    <p class="max-w-2xl mx-auto mt-4 text-xl text-gray-500 dark:text-gray-200">
                        Here are a few projects I've worked on recently. You can click on each project to learn more about it.
                    </p>
                </div>

                <div class="relative grid gap-6 mt-12 bg-top bg-cover sm:grid-cols-2 lg:grid-cols-3">
                    @foreach($pages as $page)
                        <a href="{{ route('pages.show', $page->route_name) }}" class="flex flex-col overflow-hidden transition-shadow duration-200 bg-white border shadow-lg dark:bg-gray-800 dark:border-gray-700 rounded-2xl group hover:shadow-2xl">
                            ...
                            <div class="flex flex-col justify-between p-6 space-y-4">
                                <div>
                                    <h3 class="text-xl font-bold text-gray-800 dark:text-gray-200">{{ $page->name }}</h3>
                                    <p class="mt-1 text-sm leading-5 text-gray-500 dark:text-gray-200">{{ $page->subtitle }}</p>
                                </div>
                                <div class="flex flex-wrap gap-2">
                                    @foreach(collect($page->tech_stack)->take(4) as $tech)
                                        <span class="px-2 py-1 text-xs rounded-full bg-gray-200 dark:bg-gray-400 text-gray-600 dark:text-gray-800">{{ $tech }}</span>
                                    @endforeach
                                </div>
                            </div>
                        </a>
